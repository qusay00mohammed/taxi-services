@extends('layouts.master_admin')

@section('title', 'taxi')

@section('content')

  @if (Session::has('success'))
    <div class="col-12 alert alert-success justify-content-center d-flex">
      <p class="text-center">{{ Session::get('success') }}</p>
    </div>
  @endif

  @if (Session::has('error'))
    <div class="col-12 alert alert-danger justify-content-center d-flex">
      <p class="text-center">{{ Session::get('error') }}</p>
    </div>
  @endif

  <div class="container">

    <h1 class="text-center">Manage Taxis</h1>
    <a style="margin-bottom: 10px" href="{{ route('taxis.create') }}" class="btn btn-primary"><i class="fa fa-plus"></i> Add
      New Taxi</a>
    <div class="table-responsive">
      <table class="table table-bordered text-center main-table">
        <tr>
          <td>#ID</td>
          <td>Vehicle Type</td>
          <td>The Number Of People</td>
          <td>Price</td>
          <td>Registerd Date</td>
          <td>Control</td>
        </tr>

        @foreach ($taxis as $key => $t)
          <tr>
            <td>{{ ++$key }}</td>
            <td>{{ $t->vehicleType }}</td>
            <td>{{ $t->numOfPeople }}</td>
            <td>{{ $t->price }}</td>
            <td>{{ $t->created_at->toDateString() }}</td>
            <td>
              <!--<a href="" class="btn btn-success"><i class="fa fa-edit"></i>-->
              <!--  Edit-->
              <!--</a>-->
              <a href="{{ route('taxis.destroy', $t->id) }}" class="btn btn-danger confirm"><i
                  class="fa fa-close"></i> Delete </a>

              @if ($t->regStatus == 0)
                <a href="{{ route('taxis.show', $t->id) }}" class="btn btn-info activate"><i
                    class="fa fa-check"></i>
                  activate </a>
              @endif
            </td>
          </tr>
        @endforeach

      </table>
    </div>
  </div>

@endsection
